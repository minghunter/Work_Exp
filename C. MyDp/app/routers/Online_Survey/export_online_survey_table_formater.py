...
class TableFormatter:
    ...
    def __init__(self, xlsx_name, logger):
    ...
    def format_side_axis(self, ws, is_matrix_table, start_side_row, num_format, step, lst_sub_header_col, is_tbl_sig=False):
        ...
        for irow in range(start_row, ws.max_row + 1):
            ...
            if cur2_cell.value != ws.cell(irow + 1, 2).value or cur1_cell.value != ws.cell(irow + 1, 1).value:
                ...
                if '_Group' in str(ws.cell(irow + 1, 2).value) or '_Mean' in str(ws.cell(irow + 1, 2).value) or '_Std' in str(ws.cell(irow + 1, 2).value):
                    continue
        ...
        if is_matrix_table:
            self.logger.info('Formatting %s - Brand label' % ws.title)
        ...
        for irow in range(start_row, ws.max_row):
            ...
            if irow % 500 == 0:
                self.logger.info('Formatting %s - row %s' % (ws.title, irow))
